...
{
 $query = "SELECT COUNT(*) AS TOT_ADM FROM WARD_ADMISSION_VW WHERE ADMDATE=trunc(sysdate)-1";
 $statement = $connect->prepare($query);
 $statement->execute();
 $result = $statement->fetchAll();
 //return $result;
 foreach($result as $row)
 {
  return number_format($row['TOT_ADM'], 2);
 }
...
{
 $query = "SELECT count(*) as TOT_NE FROM WARD_ADMISSION_VW WHERE ADMDATE=trunc(sysdate)-1 and ENT_NONENT='N'";
 $statement = $connect->prepare($query);
 $statement->execute();
 //return $statement->rowCount();
 $result = $statement->fetchAll();
 ...
 foreach($result as $row)
 {
  return number_format($row['TOT_NE'], 2);
 }
}
...
{
 $query = "SELECT count(*) as TOT_EN FROM WARD_ADMISSION_VW WHERE ADMDATE=trunc(sysdate)-1 and ENT_NONENT='Y'";
 $statement = $connect->prepare($query);
 $statement->execute();
 //return $statement->rowCount();
 $result = $statement->fetchAll();
 ...
 foreach($result as $row)
 {
  return number_format($row['TOT_EN'], 2);
 }
}

function count_total_product($connect)
{
 $query = "SELECT count(*) as TOT_EMP FROM WARD_ADMISSION_VW WHERE ADMDATE=trunc(sysdate)-1 and ENT_NONENT='Y' and category='99' and FAMILY='E'";
 $statement = $connect->prepare($query);
 $statement->execute();
 //return $statement->rowCount();
 $result = $statement->fetchAll();
 foreach($result as $row)
 {
  return number_format($row['TOT_EMP'], 2);
 }
}
