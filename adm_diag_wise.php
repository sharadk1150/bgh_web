...
//            date("d/m/Y", strtotime($str));
                ...
                print '<tr>'; 
                print '<td colspan="9">' . 'Provisional Diagnosis Wise Admission From : ' . date("d-m-Y", strtotime($myeid)) .  '  To Date : ' . date("d-m-Y", strtotime($myendt)) . '</td>';
                print '</tr>';
                print '<tr>';
                print '<th scope="col">Provisional DoagNosis </th>';
...
//                        }
        ...
        $query = "select (pat_provdiag1||'/'||pat_provdiag2||'/'||pat_provdiag3) as prov_diag, count(*) tot_count from ward_admission_vw_grade 
        where  to_char(admdate,'YYYY-MM-DD') between :EIDBV and :EIDBV2 group by (pat_provdiag1||'/'||pat_provdiag2||'/'||pat_provdiag3) order by 2 desc ";
